...
You don't know what the scenario is, and the sooner you find out the better.

Analyze: reveal a “situation” card to help deduce what’s going on. So it’s NOT an active shooter for example. Long-term goal to balance with short-term goals.

Analysis Pile: if you "analyze" a situation card, you add a token to the pile. At 3,5,7, and 8 you get to reveal a Scenario card. Cards with a magnifying glass can be studied.
...
Every card will have 1-3 types as a "subtitle"
...
Every situation card has:

* Name
* Types
* open/closed for line of sight (eye, closed eye)
* A study icon (or two) (magnifying glass)
* Place for art
* Row of icon boxes to handle: injury, rubble, panic, fire, chemical, delay
* Description box

Description box covers things like:
* "Rounds 3,4, & 5: add injury if not already stabilized"
* "Characters cannot exit until rubble is cleared"
* "Characters cannot exit until panic is cleared"
* "Rounds 3,4, & 5: add Fire if not already extinguished"
* "If explosion is reached, add injury to all adjacent victims"
* "If chemical spills, add injury to all surrounding victims"

## Scenario Cards

Here are some of the scenarios we could do. Here's some crazy ideas:
* Skyscraper falls down (look up split second game),
...
* Adds extra hazmats to the pile
* One big hazmat to handle at the end
...
Note: design should be such that when you flip over a card you start with cubes. A card with no cubes is "handled".

* Injury: treatment goes up each round after 3 (victim), some patients have more severe injuries and will die after a certain number of turns. Treatment can stabilize them, but then they need to be moved
* Bombs: you need to disarm a bomb by a certain time, and you can delay it but not indefinitely (hazard)
* Fires: spread and cause explosions on various flammable things (hazard)
* Backdraft: spread and cause explosions and panics (hazard)
* Rubble: just needs to be cleared, might be flammable, see-through (rubble)
...
We can definitely have two copies of each card since we also have attributes.

* Good Samaritan: moves very fast, panics easily, gets an extra trait, extra surges
...
* Hazmat team
* Hostage negotiator
* Bomb Squad: only ones with a defuse action
* Field surgeon: treats well, moves slow, needs a paramedic
* Rescue squad: good at stabilizing, clearing rubble
* Community hero: they can help reveal a level 3 card.

## Adjectives

* Keen-eyed: can observe multiple event cards in a row
* Mobile: can move quickly - double-move if in a straight line
...
Maybe you can pick up equipment if you are outside for an action. So you don't need it, but you can run back out to get it.
